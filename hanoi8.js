...
function pillar(x, y, lab) {
  ...
  this.draw = function () {
    ...
    ctx.lineTo(this.x, this.y - 400);
    ctx.stroke();
    ctx.textAlign = 'center'; // Add a semicolon after ctx.textAlign='center'
    ctx.font = '25px console';
    ctx.fillText(this.lab, this.x, this.y + 25);
  };
}
...
function moveDisk(i) {
  let disk = document.getElementById('layer' + i);
  disk.style.animation = `diskmove${i} 2s 1`; // Use a variable instead of a fixed string
}
